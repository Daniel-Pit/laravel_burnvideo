...
class ParallelConvertOne extends Command {
	 ...
	 * @var string
	 */
	protected $name = 'order:parallelConvertOne';
	 ...
	 * @var string
	 */
	protected $description = 'One Parallel DVD File Convert.';
					...
					if ($ftype == "video") {
						...
						try{
							...
							if(!file_exists($source)){
								...
								shell_exec(sprintf("s3cmd get --continue --force s3://burunvideo/%s %s", $targetfile, $source));
							}
							//file size check
							...
							if(!file_exists($source)){
								//AWS S3 Source url s3://burunvideo/dvdzip/%s/%s, $item->id, $targetfile
								shell_exec(sprintf("s3cmd get --continue --force s3://burunvideo/dvdzip/%s/%s %s", $item->id, $targetfile, $source));
							}
							...
							//fwrite($output, "error\n");
							echo ("exception:".$s3error->getMessage()."\n".$s3error->getTraceAsString());
							throw $s3error;
						}						
