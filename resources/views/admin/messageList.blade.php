...
<script>
    ...
    //console.log(searchText, userName);
    ...
    var textUser = $("#add_user").val();
            ...
            $.post(url, {'search':searchText, 'state':state, 'city':city}, function(result){
            ...
            user.uname = user.uname.replace("'", '');
            user.uname = user.uname.replace(/'/g, "");
    ...
    function onSubmitAdd()
    {
		...
		if (users.length == 0)
		{
			showError("Please select user's full name", true);
			return false;
		}
		...
		var url = "/admin/api_sendMail";
		$.post( url, { 'title':title, 'users':users, 'message':subject, 'sender': sender })
	...
	function getState()
	{
      	$('#add_user').val('');$("#candycontainer").html('');
		var url = "/admin/api_stateList";
		$.post(url, {}, function(result){
			...
			if (result.states.length > 0)
			{
				$.each( result.states, function( key, value ) {
				  state_html += "<option value='" + value.state + "'>" + value.state + "</option>";
				});
	...
	{
      	$('#add_user').val('');$("#candycontainer").html('');
		var url = "/admin/api_cityList";
		$.post(url, {state:stateCode}, function(result){
			var city_html = "<option value=''></option>";

			if (result.cities.length > 0)
			{
				$.each( result.cities, function( key, value ) {
				  city_html += "<option value='" + value.city + "'>" + value.city + "</option>";
				});
			}
    ...
    onFocusUser();
            ...
            $('#add_title').bind('focus', function(e) {
    onBlurUser();
    });
            $('#add_sender').bind('focus', function(e) {
    onBlurUser();
    });
            $('#add_message').bind('focus', function(e) {
    onBlurUser();
    });
      $('#contentarea').bind('focus', function(e) {
        //console.log("edit change");
    	onBlurUser();
    });
      $('#contentarea').on('focus', '.note-editor.note-frame', function(e) {
        //console.log("edit change");
    	onBlurUser();
    });      
 		...
 		$('#select_state').change(function(){
			...
			var selectedStateCode = $('#select_state').val();
			getCity(selectedStateCode);
			onUserInputChange();
		});
        ...
        <div class="col-xs-12">
            <div class="box">
                <div class="box-header">
                    <h3 class="box-title">Messages By Names</h3>  
                    <div class="box-tools">                                        
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <table style="width: 100%">
                    ...
                    <tr>
                        <td>
                            <input id="add_user" name="add_user" class="form-control" >
                            <div id="candycontainer" class="candycontainer">
